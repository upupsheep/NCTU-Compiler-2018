...
int hasError = 0; // default is 0(no error)
int constRename = 0;
int varRename = 0;
...
funct_def:
	scalar_type ID L_PAREN R_PAREN 
	{
		...
		{
			// check return type(scalar_type)
			if(!checkIsScalarType($1)){
				printf("##########Error at Line #%d: Return type must be scalar type.##########\n", linenum);
				hasError = 1;
			} else {
				struct SymTableNode *newNode = createFunctionNode(0, $2,scope,funcReturnType,NULL);
				insertTableNode(symbolTableList->global,newNode);
			}			
		} else {
			// check if match declaration type or not
			if(node->fromDeclare){
				if(node->type->baseType!=$1){
					printf("##########Error at Line #%d: Wrong return type.##########\n", linenum);
					hasError = 1;
				}
				if(node->attr!=NULL){
					printf("##########Error at Line #%d: Wrong parameter type.##########\n", linenum);
					hasError = 1;
				}
			} else { // means it's redefined
				printf("##########Error at Line #%d: %s has be redefined.##########\n", linenum, $2);
				hasError = 1;				
			}			
		}
		free($2);
	...
	| scalar_type ID L_PAREN parameter_list R_PAREN 
	{
			...
			if(node==NULL)//no declaration yet
			{
				...
				if(!checkIsScalarType($1)){
					...
					struct SymTableNode *newNode = createFunctionNode(0, $2,scope,funcReturnType,attr);
					insertTableNode(symbolTableList->global,newNode);
				}				
			} else {
				// check if match declaration type or not
	...
	L_BRACE 
		{//enter a new scope
			...
			if(node->fromDeclare){
				if(node->attr->funcParam->head!=attrNode){
					printf("##########Error at Line #%d: Wrong parameter type.##########\n", linenum);
					hasError = 1;
				} else {
					while(attrNode!=NULL)
	...
	//add parameters
		...
		if(node->fromDeclare){
			...
			if(node->attr->funcParam->head->value->baseType!=attrNode->value->baseType){
				printf("##########Error at Line #%d: Wrong parameter type.##########\n", linenum);
				hasError = 1;
			} else {
				while(attrNode!=NULL)
...
funct_decl:
	...
	{
		if(checkHasDecl_Def(symbolTableList->global, $2, $1)){
			printf("##########Error at Line #%d: %s has been redeclared.##########\n", linenum, $2);
			hasError = 1;
		} else {
			funcReturnType = createExtType($1,0,NULL);
			struct SymTableNode *newNode = createFunctionNode(1, $2,scope,funcReturnType,NULL);
			insertTableNode(symbolTableList->global,newNode);
			free($2);
		}		
	}
	| scalar_type ID L_PAREN parameter_list R_PAREN SEMICOLON
	{
		if(checkHasDecl_Def(symbolTableList->global, $2, $1)){
			...
			funcReturnType = createExtType($1,0,NULL);
			struct Attribute *attr = createFunctionAttribute($4);
			struct SymTableNode *newNode = createFunctionNode(1, $2,scope,funcReturnType,attr);
			insertTableNode(symbolTableList->global,newNode);
			free($2);
		}		
	}
	...
	{
		if(checkHasDecl_Def(symbolTableList->global, $2, VOID_t)){
			printf("##########Error at Line #%d: %s has been redeclared.##########\n", linenum, $2);
			hasError = 1;
		} else {
			funcReturnType = createExtType(VOID_t,0,NULL);
...
identifier_list:
	identifier_list COMMA ID
	{
		varRename = 0;
		struct SymTableNode* node;
		if(scope==0) node = findVarDeclaration(symbolTableList->global,$3);
		else node = findVarDeclaration(symbolTableList->tail,$3);
		if(node!=NULL){ // redeclared
			printf("##########Error at Line #%d: Variable %s has been declared.##########\n",linenum,$3);
			hasError = 1;
			varRename = 1;
			free($3);
		} else {
	...
	| ID ASSIGN_OP logical_expression
	{
		varRename = 0;
		struct SymTableNode* node;
		if(scope==0) node = findVarDeclaration(symbolTableList->global,$1);
		else node = findVarDeclaration(symbolTableList->tail,$1);
		if(node!=NULL){ // redeclared
			printf("##########Error at Line #%d: Variable %s has been declared.##########\n",linenum,$1);
			hasError = 1;
			varRename = 1;
...
const_list:
	const_list COMMA ID ASSIGN_OP literal_const
	{
		...
		struct symTableNode* node;
		if(scope==0) node = findConstDeclaration(symbolTableList->global,$3);
		else node = findConstDeclaration(symbolTableList->tail,$3);
		if(node!=NULL){ // has been declared
			printf("##########Error at Line #%d: Constant %s has been declared.##########\n",linenum,$3);
			hasError = 1;
			constRename = 1;
		} else {
	...
	| ID ASSIGN_OP literal_const
    {
		constRename = 0;
		struct symTableNode* node;
		if(scope==0) node = findConstDeclaration(symbolTableList->global,$1);
		else node = findConstDeclaration(symbolTableList->tail,$1);
		if(node!=NULL){ // has been declared
			// printf("redeclared\n");
			// printf("%s\n", $1);
			printf("##########Error at Line #%d: Constant %s has been declared.##########\n",linenum,$1);
...
function_invoke_statement:
	...
	{
		if(findFuncDeclaration(symbolTableList->global, $1)==NULL){
			printf("##########Error at Line #%d: Unknown function of %s.##########\n", linenum, $1);
			hasError = 1;
		}
		free($1);
	}
