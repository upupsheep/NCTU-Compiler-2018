...
struct SymTableNode* deleteTableNode(
    struct SymTableNode* target)  // return next node
{
    ...
    if (target == NULL)
        ...
        if (target->attr != NULL) killAttribute(target->attr);
        free(target);
    }
    return next;
}

struct SymTableNode* createVariableNode(const char* name, int level,
                                        struct ExtType* type) {
    struct SymTableNode* newNode =
        (struct SymTableNode*)malloc(sizeof(struct SymTableNode));
    // set node
    newNode->fromDeclare = 1;
...
struct SymTableNode* createFunctionNode(int fromDeclare, const char* name,
                                        int level, struct ExtType* type,
                                        struct Attribute* attr) {
    struct SymTableNode* newNode =
        (struct SymTableNode*)malloc(sizeof(struct SymTableNode));
    // set node
    newNode->fromDeclare = fromDeclare;
    strncpy(newNode->name, name, 32);
...
struct SymTableNode* createConstNode(const char* name, int level,
                                     struct ExtType* type,
                                     struct Attribute* attr) {
    struct SymTableNode* newNode =
        (struct SymTableNode*)malloc(sizeof(struct SymTableNode));
...
struct SymTableNode* createParameterNode(const char* name, int level,
                                         struct ExtType* type) {
    struct SymTableNode* newNode =
        (struct SymTableNode*)malloc(sizeof(struct SymTableNode));
    // set node
...
int killAttribute(struct Attribute* target) {
    ...
    if (target->constVal != NULL) {
        ...
        if (target->constVal->reference > 0) return -1;
    ...
    if (target->funcParam != NULL) {
        ...
        if (target->funcParam->reference > 0) return -1;
...
struct FuncAttrNode* createFuncAttrNode(struct ExtType* type,
                                        const char* name) {
    struct FuncAttrNode* newNode =
        (struct FuncAttrNode*)malloc(sizeof(struct FuncAttrNode));
    /*reference*/
    newNode->value = type;
    type->reference += 1;
    /*         */
...
int connectFuncAttrNode(
    struct FuncAttrNode* head,
    struct FuncAttrNode*
        newNode)  // connect node to tail of function attribute list
{
...
struct ExtType* createExtType(BTYPE baseType, bool isArray,
                              struct ArrayDimNode* dimArray) {
    int dimNum = 0;
    struct ArrayDimNode* temp = dimArray;
    struct ExtType* newExtType =
        (struct ExtType*)malloc(sizeof(struct ExtType));
...
int connectArrayDimNode(
    struct ArrayDimNode* head,
    struct ArrayDimNode* newNode)  // connect dimension node to tail of list
{
    if (head == NULL || newNode == NULL || head == newNode) return -1;
...
struct SymTableNode* findFuncDeclaration(struct SymTable* table,
                                         const char* name) {
...
int printSymTable(struct SymTable* table) {
    ...
    printf(
        "======================================================================"
        "=================\n");
    // Name [29 blanks] Kind [7 blanks] Level [7 blank] Type [15 blanks]
    // Attribute [15 blanks]
    printf(
...
int connectVariableList(struct VariableList* list, struct Variable* node) {
    ...
    if (node->next != NULL) return -2;
    if (list->tail != list->head) list->tail->reference -= 1;
    /**/
    list->tail->next = node;
...
//--------------------------
int checkHasDecl_Def(struct SymTable* table, const char* name,
                     BTYPE return_type) {
    // char strBuffer[20];
    struct SymTableNode* node;
    node = findFuncDeclaration(table, name);
    if (node == NULL) return 0;
    // if (node->type->baseType == return_type) {
    //     return 1;
    // } else
    //     return 0;
    return 1;
...
int checkIsScalarType(BTYPE retType) {
    if (retType != INT_t && retType != FLOAT_t && retType != DOUBLE_t &&
        retType != BOOL_t && retType != VOID_t) {
        return 0;
    }
    return 1;
}

struct SymTableNode* findConstDeclaration(struct SymTable* table,
                                          const char* name) {
    struct SymTableNode* temp = table->head;
    while (temp != NULL) {
...
struct SymTableNode* findVarDeclaration(struct SymTable* table,
                                        const char* name) {
    struct SymTableNode* temp = table->head;
    while (temp != NULL) {
