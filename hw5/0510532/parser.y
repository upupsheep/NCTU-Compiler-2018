...
int inloop = 0;
__BOOLEAN inMainFunction = __FALSE;
...
funct_def: scalar_type ID L_PAREN R_PAREN 
			{
				funcReturn = $1; 
				struct SymNode *node;
				inMainFunction = (strcmp($2, "main") == 0) ? __TRUE : __FALSE;
				node = findFuncDeclaration( symbolTable, $2 );
				if( node != 0 ){
					verifyFuncDeclaration( symbolTable, 0, $1, node );
				}
				else{
					insertFuncIntoSymTable( symbolTable, $2, 0, $1, scope, __TRUE, inMainFunction );
				}
				genFuncDecl($2, NULL, $1);
				limitStack();
				genReadStream();
			}
			compound_statement { funcReturn = 0; genEndFunc($1);}	
				...
				// check and insert function into symbol table
					...
					if( node != 0 ){
						if(verifyFuncDeclaration( symbolTable, $4, $1, node ) == __TRUE){	
							insertParamIntoSymTable( symbolTable, $4, scope+1, inMainFunction );
						}				
					}
					else{
						insertFuncIntoSymTable( symbolTable, $2, $4, $1, scope, __TRUE, inMainFunction );
						insertParamIntoSymTable( symbolTable, $4, scope+1, inMainFunction );
					}
				}
				genFuncDecl($2, $4, $1);
		  ...
		  | VOID ID L_PAREN R_PAREN 
			{
				...
				if( node != 0 ){
					...
					insertFuncIntoSymTable( symbolTable, $2, 0, createPType( VOID_t ), scope, __TRUE, inMainFunction);	
				}
				struct PType pt;
				pt.type = VOID_t;
				genFuncDecl($2, NULL, &pt);
				limitStack();
				genReadStream();
			}
			compound_statement { 
			...
			{									
				...
				// check and insert function into symbol table
					...
					if( node != 0 ){
						...
						insertFuncIntoSymTable( symbolTable, $2, $4, createPType( VOID_t ), scope, __TRUE, inMainFunction );
						insertParamIntoSymTable( symbolTable, $4, scope+1, inMainFunction );	
					}
				}
				struct PType pt;
...
funct_decl: scalar_type ID L_PAREN R_PAREN SEMICOLON
			{
				insertFuncIntoSymTable( symbolTable, $2, 0, $1, scope, __FALSE, inMainFunction );	
			}
		   | scalar_type ID L_PAREN parameter_list R_PAREN SEMICOLON
		    {
				...
				if( paramError == __TRUE ){
					...
					insertFuncIntoSymTable( symbolTable, $2, $4, $1, scope, __FALSE, inMainFunction);
				}
			}
		   | VOID ID L_PAREN R_PAREN SEMICOLON
			{				
				insertFuncIntoSymTable( symbolTable, $2, 0, createPType( VOID_t ), scope, __FALSE, inMainFunction );
			}
		   | VOID ID L_PAREN parameter_list R_PAREN SEMICOLON
			{
				...
				if( paramError == __TRUE ){
					...
					insertFuncIntoSymTable( symbolTable, $2, $4, createPType( VOID_t ), scope, __FALSE, inMainFunction );
				}
			}
		   ;
...
var_decl: scalar_type identifier_list SEMICOLON
			{
				...
				int i;
				struct varDeclParam *ptrArr[256];
				int len;
				for( len=0, ptr=$2 ; ptr!=0 ; len++, ptr=(ptr->next) ) {
					ptrArr[len] = ptr;
				}
				...
				for( ptr=$2 ; ptr!=0 ; ptr=(ptr->next) ) {						
					if( verifyRedeclaration( symbolTable, ptr->para->idlist->value, scope ) == __FALSE ) { }
						...
						if( verifyVarInitValue( $1, ptr, symbolTable, scope ) ==  __TRUE ){	
							...
							insertTab( symbolTable, newNode , inMainFunction);
							if(scope == 0){
								genGlobalVar(newNode->name, $1);
							}
						}
					}
				}
				
				struct expr_sem *exp1 = malloc(sizeof(struct expr_sem ));
				struct var_ref_sem *varRef = malloc(sizeof(struct var_ref_sem));

				for(i = 0, ptr=$2 ; ptr!=0 ; i++,ptr=(ptr->next) ) {
					if(ptrArr[len-1-i]->expr != NULL){
						exp1->varRef = varRef;
						exp1->varRef->id = ptrArr[len-1-i]->para->idlist->value;
						exp1->pType = ptrArr[len-1-i]->para->pType;
						genVarStore(exp1, ptrArr[len-1-i]->expr, symbolTable, scope);
					}
				}
...
simple_statement: variable_reference ASSIGN_OP logical_expression SEMICOLON
					{
						...
						genVarStore( $1, $3, symbolTable, scope);
					}
				 | PRINT logical_expression SEMICOLON {

				 	genPrint($2, symbolTable, scope);
				 	verifyScalarExpr( $2, "print" ); 
				 }
				 | READ variable_reference SEMICOLON 
					{ 
						if( verifyExistence( symbolTable, $2, scope, __TRUE ) == __TRUE ){					verifyScalarExpr( $2, "read" ); 
							genRead($2, symbolTable, scope);
						}
					}
				 ;
...
conditional_statement: IF L_PAREN conditional_if  R_PAREN compound_statement {
					  ...
					  ;
conditional_if: logical_expression { verifyBooleanExpr( $1, "if" ); genCondition();};				  

				
while_statement: WHILE {genWhile();} L_PAREN logical_expression {genCondition();verifyBooleanExpr( $4, "while" ); } R_PAREN { inloop++; }
					compound_statement { genGotoWhileLabel(); genElseLabel(1); inloop--; }
				| { inloop++; } DO {genWhile();} compound_statement WHILE L_PAREN logical_expression {genCondition();genGotoWhileLabel(); genElseLabel(1);} R_PAREN SEMICOLON  
					{ 
...
for_statement: {genFor();} FOR L_PAREN initial_expression SEMICOLON {genForConditionLabel();} control_expression {genForConditionEnd(); } SEMICOLON {genForIncrLabel();}increment_expression {genForIncrEnd();} R_PAREN  { genForBodyLabel();inloop++; }
					compound_statement  { inloop--; genForBodyEnd(); }
			  ;
