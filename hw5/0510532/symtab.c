...
void insertTab( struct SymTable *table, struct SymNode *newNode, __BOOLEAN inMainFunction )
{
	int location = 0;
	if( table->entry[location] == 0 ) {	// the first
		if(newNode->scope != 0){
			table -> localVarCount = (inMainFunction) ? 1: 0;
			if(newNode->type->type != CONSTANT_t){
				newNode->localNumber = table -> localVarCount;
				table->localVarCount++;
			}
		}
		else
		...
		if(nodePtr->scope == 0 && newNode->scope == 1){
			table -> localVarCount = (inMainFunction) ? 1: 0;
		}
		if(newNode->scope != 0){
			if(newNode->type->type != CONSTANT_t){
