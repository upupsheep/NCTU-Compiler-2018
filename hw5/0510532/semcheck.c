...
__BOOLEAN insertParamIntoSymTable( struct SymTable *table, struct param_sem *params, int scope, __BOOLEAN inMainFunction )
{
			...
			if( parPtr->pType->isError == __TRUE ) { 
				...
				for( idPtr=(parPtr->idlist) ; idPtr!=0 ; idPtr=(idPtr->next) ) {
					if( verifyRedeclaration( table, idPtr->value, scope ) ==__FALSE ) { result = __TRUE;  }
						...
						insertTab( table, newNode, inMainFunction );
					}
				}
			}
		} 
...
void insertFuncIntoSymTable( struct SymTable *table, const char *id, struct param_sem *params, struct PType *retType, int scope, __BOOLEAN isDefine, __BOOLEAN inMainFunction  )
{
