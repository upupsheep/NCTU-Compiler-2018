...
struct idNode_sem *createIdList(const char *str) {
    struct idNode_sem *newNode =
        (struct idNode_sem *)malloc(sizeof(struct idNode_sem));
...
struct ConstAttr *createConstAttr(SEMTYPE type, void *value) {
    struct ConstAttr *result =
        (struct ConstAttr *)malloc(sizeof(struct ConstAttr));
    ...
    switch (type) {
        case INTEGER_t:
            ...
            if (*(int *)value < 0) result->hasMinus = __TRUE;
            break;
        case FLOAT_t:
            result->value.floatVal = *(float *)value;
            if (*(float *)value < 0.0) result->hasMinus = __TRUE;
            break;
        case STRING_t:
        ...
        case DOUBLE_t:
            ...
            if (*(double *)value < 0.0) result->hasMinus = __TRUE;
            break;
    }
    return result;
}

struct constParam *createConstParam(struct ConstAttr *constNode,
                                    const char *id) {
    struct constParam *result =
        (struct constParam *)malloc(sizeof(struct constParam));
...
struct param_sem *createParam(struct idNode_sem *ids, struct PType *pType) {
    struct param_sem *result =
        (struct param_sem *)malloc(sizeof(struct param_sem));
    result->idlist = ids;
    result->pType = pType;
    result->next = 0;
...
struct varDeclParam *createVarDeclParam(struct param_sem *par,
                                        struct expr_sem *exp) {
    struct varDeclParam *result =
        (struct varDeclParam *)malloc(sizeof(struct varDeclParam));
    result->para = par;
    result->expr = exp;
...
void increaseDim(struct expr_sem *expr, SEMTYPE dimType) {
    struct typeNode *newNode =
        (struct typeNode *)malloc(sizeof(struct typeNode));
    ...
    if (expr->varRef->dim == 0) {  // the first dim
        ...
        for (typePtr = (expr->varRef->dim); (typePtr->next) != 0;
...
void increaseArrayDim(struct PType *pType, int lo, int hi) {
    ...
    struct ArrayDimNode *newDim =
        (struct ArrayDimNode *)malloc(sizeof(struct ArrayDimNode));
    newDim->low = lo;
    newDim->high = hi;
    newDim->size = hi - lo;
    newDim->next = 0;
...
struct ArrayDimNode *copyArrayDimList(struct ArrayDimNode *src) {
    if (src == 0) {
        ...
        struct ArrayDimNode *ptr =
            dest;  // ptr: points to the last element of new list
        struct ArrayDimNode *arrPtr;
        for (arrPtr = (src->next); arrPtr != 0; arrPtr = (arrPtr->next)) {
            struct ArrayDimNode *newNode =
                (struct ArrayDimNode *)malloc(sizeof(struct ArrayDimNode));
...
void verifyArrayType(struct idNode_sem *ids, struct PType *pType) {
    ...
    if (pType->isError == __TRUE) {
        fprintf(stdout,
                "########## Error at Line#%d: wrong dimension declaration for "
                "array ",
                linenum);
        semError = __TRUE;
        printf("%s", ids->value);
...
SEMTYPE verifyArrayIndex(struct expr_sem *expr) {
    ...
    if (expr->isDeref == __FALSE) {
        ...
        fprintf(stdout,
                "########## Error at Line#%d: array index cannot be arrya_type "
                "##########\n",
                linenum);
        semError = __TRUE;
        result = ERROR_t;
...
__BOOLEAN verifyRedeclaration(struct SymTable *table, const char *str,
                              int scope) {
    __BOOLEAN result = __TRUE;
    // then check normal variable(s)
    if (lookupSymbol(table, str, scope, __TRUE) ==
        0) {  // only search current scope
        result = __TRUE;
    } else {
        fprintf(
            stdout,
            "########## Error at Line#%d: symbol %s is redeclared ##########\n",
            linenum, str);
        semError = __TRUE;
        result = __FALSE;
    }
...
__BOOLEAN verifyExistence(struct SymTable *table, struct expr_sem *expr,
                          int scope, __BOOLEAN isAssignmentLHS) {
    __BOOLEAN result = __TRUE;
    struct SymNode *node = 0;

    node = lookupSymbol(table, expr->varRef->id, scope,
                        __FALSE);  // if not found, check normal symbol

    if (node == 0) {  // symbol not found
        fprintf(
            stdout,
            "########## Error at Line#%d: '%s' is not declared ##########\n",
        ...
        if (node->category == FUNCTION_t) {
            ...
            fprintf(stdout,
                    "########## Error at Line#%d: constant '%s' cannot be "
                    "assigned ##########\n",
                    linenum, node->name);
            semError = __TRUE;
            expr->pType = createPType(ERROR_t);
            ...
            if (expr->varRef->dimNum == 0) {
                ...
                if (node->type->dimNum < expr->varRef->dimNum) {
                    fprintf(
                        stdout,
                        "########## Error at Line#%d: '%s' is %d dimension(s), "
                        "but reference in %d dimension(s) ##########\n",
                        linenum, node->name, node->type->dimNum,
                        expr->varRef->dimNum);
                    semError = __TRUE;
                    expr->pType = createPType(ERROR_t);
                    result = __FALSE;
                } else if (node->type->dimNum ==
                           expr->varRef->dimNum) {  // result in scalar!
                    ...
                    int i;
                    struct ArrayDimNode *arrPtr;
                    for (i = 0, arrPtr = (node->type->dim);
                         i < (expr->varRef->dimNum);
...
/**
 * void verifyUnaryMinus: mark the subexpression with beginning unaryOp, and
 * check type mismatch
 */
void verifyUnaryMinus(struct expr_sem *expr) {
    ...
    if (expr->isDeref == __FALSE) {
        ...
        fprintf(stdout,
                "########## Error at Line#%d: operand of unary - is not "
                "integer/real ##########\n",
                linenum);
        semError = __TRUE;
        expr->isDeref = __TRUE;
        ...
        if (!(((expr->pType->type) == INTEGER_t) ||
              ((expr->pType->type) == FLOAT_t) ||
              ((expr->pType->type) == DOUBLE_t))) {
            fprintf(stdout,
                    "########## Error at Line#%d: operand of unary - is not "
                    "integer/real ##########\n",
                    linenum);
...
void verifyUnaryNOT(struct expr_sem *expr) {
    ...
    if (expr->pType->dimNum != 0) {
        fprintf(stdout,
                "########## Error at Line#%d: operand of 'not' cannot be "
                "array_type ##########\n",
                linenum);
        semError = __TRUE;
        expr->pType->type = ERROR_t;
        ...
        fprintf(stdout,
                "########## Error at Line#%d: operand of 'not' is not boolean "
                "##########\n",
                linenum);
        semError = __TRUE;
        expr->pType->type = ERROR_t;
...
void verifyAssignmentTypeMatch(struct expr_sem *LHS, struct expr_sem *RHS) {
    ...
    if ((LHS->pType->type) == ERROR_t ||
        (RHS->pType->type) == ERROR_t) {  // previous error(s)
                                          // ......
    }
    // verify type
    else if (LHS->pType->type != RHS->pType->type) {
        if (!((LHS->pType->type == FLOAT_t || LHS->pType->type == DOUBLE_t) &&
              RHS->pType->type == INTEGER_t)) {
            if (!(LHS->pType->type == DOUBLE_t &&
                  RHS->pType->type == FLOAT_t)) {
                misMatch = __TRUE;
            }
        }
    }
    ...
    if (misMatch == __TRUE) {
        fprintf(stdout,
                "########## Error at Line#%d: type mismatch, LHS= ", linenum);
        semError = __TRUE;
...
__BOOLEAN verifyVarInitValue(struct PType *scalar, struct varDeclParam *var,
    ...
    if (var->isInit == __TRUE) {
        if (var->isArray == __TRUE) {
            ...
            if (var->expr->isDeref == __FALSE) {
                if (verifyExistence(table, var->expr, scope, __FALSE) ==
                    __FALSE) {
                    return __FALSE;
                }
            }
            if (var->expr->pType->type != scalar->type) {
                if (!((scalar->type == FLOAT_t || scalar->type == DOUBLE_t) &&
                      var->expr->pType->type ==
                          INTEGER_t)) {  // float|double  = int
                    if (!(scalar->type == DOUBLE_t &&
                          var->expr->pType->type ==
                              FLOAT_t)) {  // double = float
                        result = __FALSE;
                        fprintf(stdout,
                                "########## Error at Line#%d: initial value "
                                "type error ##########\n",
                                linenum);
                        semError = __TRUE;
                    }
                ...
                result = __FALSE;
                fprintf(stdout,
                        "########## Error at Line#%d: initial value type(array "
                        "type) error ##########\n",
                        linenum);
                semError = __TRUE;
            }
...
__BOOLEAN verifyArrayInitVal(struct PType *scalar, struct varDeclParam *var,
                             struct SymTable *table, int scope) {
    __BOOLEAN result = __TRUE;
    ...
    for (tmp = var->expr; tmp != 0; tmp = (tmp->next)) {
        ...
        if (tmp->pType->type == ERROR_t) {
            ...
            if (var->para->pType->type != tmp->pType->type) {  // type different
                if (!((scalar->type == FLOAT_t || scalar->type == DOUBLE_t) &&
                      tmp->pType->type == INTEGER_t)) {  // float|double  = int
                    if (!(scalar->type == DOUBLE_t &&
                          tmp->pType->type == FLOAT_t)) {  // double = float
                        result = __FALSE;
                        fprintf(stdout,
                                "########## Error at Line#%d: initial value "
                                "type error ##########\n",
                ...
                var->para->pType->type = ERROR_t;
                fprintf(stdout,
                        "########## Error at Line#%d: initial value can't be "
                        "array type ##########\n",
                        linenum);
                semError = __TRUE;
                result = __FALSE;
    ...
    if (initElement > element) {
        var->para->pType->type = ERROR_t;
        fprintf(stdout,
                "########## Error at Line#%d: too many initial value numbers "
                "##########\n",
                linenum);
        semError = __TRUE;
        result = __FALSE;
...
void verifyModOp(struct expr_sem *op1, struct expr_sem *op2) {
    if ((op1->pType->type) == ERROR_t || (op2->pType->type) == ERROR_t) {
        ...
        fprintf(stdout, "########## Error at Line#%d: adjacent operator 'mod",
                linenum);
        semError = __TRUE;
        fprintf(stdout, "' and '");
        ...
        fprintf(stdout,
                "########## Error at Line#%d: operand(s) between 'mod' are "
                "array_type\n",
                linenum);
        semError = __TRUE;
        op1->pType->type = ERROR_t;
    } else if ((op1->pType->type) != INTEGER_t ||
               (op2->pType->type) != INTEGER_t) {
        fprintf(stdout,
                "########## Error at Line#%d: operand(s) between 'mod' are not "
                "integer\n",
                linenum);
        semError = __TRUE;
        op1->pType->type = ERROR_t;
...
void verifyArithmeticOp(struct expr_sem *op1, OPERATOR operator,
                        struct expr_sem *op2) {
    if ((op1->pType->type) == ERROR_t || (op2->pType->type) == ERROR_t) {
        op1->pType->type = ERROR_t;
    } else if (op2->beginningOp != NONE_t) {
        fprintf(stdout, "########## Error at Line#%d: adjacent operator '",
                linenum);
        semError = __TRUE;
        ...
        fprintf(stdout, "########## Error at Line#%d: operand(s) between '",
                linenum);
        semError = __TRUE;
        printOperator(operator);
        ...
        if (op1->pType->type == STRING_t &&
            op2->pType->type == STRING_t) {  // string concatenation
            fprintf(stdout, "########## Error at Line#%d: operand(s) between '",
                    linenum);
            semError = __TRUE;
            printOperator(operator);
            fprintf(stdout, "' are string type ##########\n");
                    ...
                    (op2->pType->type == INTEGER_t ||
                     op2->pType->type == FLOAT_t ||
                     op2->pType->type ==
                         DOUBLE_t))) {  // need to consider type coercion
            if (op1->pType->type == INTEGER_t &&
                op2->pType->type == INTEGER_t) {
                op1->pType->type = INTEGER_t;
            } else {
                if (op1->pType->type == DOUBLE_t ||
                    op2->pType->type == DOUBLE_t) {
                    op1->pType->type = DOUBLE_t;
...
void verifyRelOp(struct expr_sem *op1, OPERATOR operator,
                 struct expr_sem *op2) {
    if ((op1->pType->type) == ERROR_t || (op2->pType->type) == ERROR_t) {
        op1->pType->type = ERROR_t;
    } else if (op2->beginningOp != NONE_t) {
        ...
        op1->pType->type = ERROR_t;
                 ...
                 (op2->pType->type == INTEGER_t ||
                  op2->pType->type == FLOAT_t ||
                  op2->pType->type == DOUBLE_t))) {
        if (op1->pType->type == BOOLEAN_t && op2->pType->type == BOOLEAN_t &&
            ...
            printOperator(operator);
            fprintf(stdout,
                    "' are not integer/real or the same type##########\n");
            op1->pType->type = ERROR_t;
        }
    } else {  // pass verification, result is boolean!
...
void verifyAndOrOp(struct expr_sem *op1, OPERATOR operator,
                   struct expr_sem *op2) {
    if ((op1->pType->type) == ERROR_t || (op2->pType->type) == ERROR_t) {
        op1->pType->type = ERROR_t;
    } else if (op2->beginningOp != NONE_t) {
...
struct expr_sem *verifyFuncInvoke(const char *id, struct expr_sem *exprList,
    ...
    if (node == 0) {  // symbol not found
        fprintf(
            stdout,
            "########## Error at Line#%d: symbol '%s' not found ##########\n",
            linenum, id);
        semError = __TRUE;
        result->pType = createPType(ERROR_t);
    } else if (node->category != FUNCTION_t) {
        fprintf(stdout,
                "########## Error at Line#%d: symbol '%s' is not a function "
                "##########\n",
                linenum, id);
        semError = __TRUE;
        result->pType = createPType(ERROR_t);
        ...
        if (node->attribute->formalParam->paramNum == 0) {
            if (exprList != 0) {
                fprintf(stdout,
                        "########## Error at Line#%d: too many arguments to "
                        "function %s ##########\n",
                        linenum, node->name);
                semError = __TRUE;
                result->pType = createPType(ERROR_t);
            } else {
                // result->pType = node->type;	// return type of function
                // declaration
            ...
            for (listPtr = (node->attribute->formalParam->params),
                exprPtr = exprList;
                 (listPtr != 0) && (exprPtr != 0);
                 listPtr = (listPtr->next), exprPtr = (exprPtr->next)) {
                // verify type
                if (listPtr->value->type != exprPtr->pType->type) {
                    if (!((listPtr->value->type == FLOAT_t) &&
                          (exprPtr->pType->type == INTEGER_t)) &&
                        !((listPtr->value->type == DOUBLE_t) &&
                          (exprPtr->pType->type == INTEGER_t)) &&
                        !((listPtr->value->type == DOUBLE_t) &&
                          (exprPtr->pType->type == FLOAT_t)))
                        mismatch = __TRUE;
                }
                // verify dimension #
                if (listPtr->value->dimNum != exprPtr->pType->dimNum) {
                    ...
                    for (lhsPtr = (listPtr->value->dim),
                        rhsPtr = (exprPtr->pType->dim);
                         lhsPtr != 0;
            ...
            if (mismatch == __TRUE) {
                ...
                if (listPtr != 0) {
                    fprintf(stdout,
                            "########## Error at Line#%d: too few arguments to "
                            "function '%s' ##########\n",
                            linenum, node->name);
                    semError = __TRUE;
                    result->pType = createPType(ERROR_t);
                    ...
                    fprintf(stdout,
                            "########## Error at Line#%d: too many arguments "
                            "to function '%s' ##########\n",
                            linenum, node->name);
                    semError = __TRUE;
                    result->pType = createPType(ERROR_t);
...
void verifyScalarExpr(struct expr_sem *expr, const char *str) {
    if (expr->pType->dim > 0) {
        fprintf(stdout,
                "########## Error at Line#%d: %s statement's operand is array "
                "type ##########\n",
                linenum, str);
        semError = __TRUE;
    }
...
void verifyBooleanExpr(struct expr_sem *expr, const char *str) {
    if (expr->pType->dim > 0) {
        ...
        fprintf(stdout,
                "########## Error at Line#%d: %s statement's operand is not "
                "boolean type ##########\n",
                linenum, str);
        semError = __TRUE;
    }
...
void verifyReturnStatement(struct expr_sem *expr, struct PType *funcReturn) {
    if (funcReturn == 0) {
        fprintf(stdout,
                "########## Error at Line#%d: program cannot be returned "
                "##########\n",
                linenum);
        semError = __TRUE;
    } else if (funcReturn->type == VOID_t) {
        fprintf(stdout,
                "########## Error at Line#%d: void function cannot be returned "
                "##########\n",
                linenum);
        semError = __TRUE;
    } else if (funcReturn->type != expr->pType->type) {
        if (!((funcReturn->type == FLOAT_t || funcReturn->type == DOUBLE_t) &&
              expr->pType->type == INTEGER_t)) {
            if (!(funcReturn->type == DOUBLE_t &&
                  expr->pType->type == FLOAT_t)) {
                fprintf(stdout,
        ...
        fprintf(stdout,
                "########## Error at Line#%d: return dimension number mismatch "
                "##########\n",
                linenum);
        semError = __TRUE;
    } else {
        ...
        int i;
        for (returnDim = (funcReturn->dim), exprDim = (expr->pType->dim), i = 0;
             returnDim != 0;
             returnDim = (returnDim->next), exprDim = (exprDim->next), ++i) {
            if (returnDim->size != exprDim->size) {
                fprintf(stdout,
                        "########## Error at Line#%d: return dimension #%d's "
                        "size mismatch ##########\n",
                        linenum, i);
                semError = __TRUE;
            }
...
__BOOLEAN insertParamIntoSymTable(struct SymTable *table,
                                  struct param_sem *params, int scope) {
        ...
        for (parPtr = params; parPtr != 0; parPtr = (parPtr->next)) {
            if (parPtr->pType->isError == __TRUE) {
                ...
                for (idPtr = (parPtr->idlist); idPtr != 0;
                     idPtr = (idPtr->next)) {
                    if (verifyRedeclaration(table, idPtr->value, scope) ==
                        __FALSE) {
                        result = __TRUE;
                    } else {  // without error, insert into symbol table
                        newNode =
                            createParamNode(idPtr->value, scope, parPtr->pType);
...
void insertFuncIntoSymTable(struct SymTable *table, const char *id,
                            struct param_sem *params, struct PType *retType,
                            int scope, __BOOLEAN isDefine) {
    if (verifyRedeclaration(table, id, scope) == __FALSE) {
        return;
    } else {
        struct FuncAttr *formalParam =
            (struct FuncAttr *)malloc(sizeof(struct FuncAttr));
        ...
        if (params == 0) {
            ...
            struct PTypeList *lastPtr =
                0;  // incicate the last element in PType list

            for (parPtr = params; parPtr != 0; parPtr = (parPtr->next)) {
                for (idPtr = (parPtr->idlist); idPtr != 0;
                     idPtr = (idPtr->next)) {
                    if (formalParam->paramNum == 0) {  // add the first entry
                        formalParam->params = (struct PTypeList *)malloc(
                            sizeof(struct PTypeList));
                        ...
                        struct PTypeList *newPTypeList =
                            (struct PTypeList *)malloc(
                                sizeof(struct PTypeList));
        ...
        struct SymNode *newNode =
            createFuncNode(id, scope, retType, formalParam);
...
__BOOLEAN verifyFuncDeclaration(struct SymTable *table, struct param_sem *par,
                                struct PType *scalarType,
                                struct SymNode *node) {
    __BOOLEAN result = __TRUE;

    if (node->isFuncDefine == __TRUE) {
        fprintf(stdout,
                "########## Error at Line#%d: function '%s' has been defined "
                "##########\n",
                linenum, node->name);
        semError = __TRUE;
        result = __FALSE;
        ...
        fprintf(
            stdout,
            "########## Error at Line#%d: name '%s' has been used ##########\n",
            linenum, node->name);
        semError = __TRUE;
        result = __FALSE;
    } else {  // check parameters...
        if (scalarType->type != node->type->type) {
            fprintf(stdout,
                    "########## Error at Line#%d: %s function return type "
                    "error ##########\n",
                    linenum, node->name);
            semError = __TRUE;
            result = __FALSE;
            ...
            __BOOLEAN mismatch = __FALSE;
            struct PTypeList
                *listPtr;  // = node->attribute->formalParam->params;
...
void checkUndefinedFunc(struct SymTable *symtab) {
    for (int i = 0; i < HASHBUNCH; ++i) {
        if (symtab->entry[i] != NULL) {
            if (symtab->entry[i]->category == FUNCTION_t &&
                symtab->entry[i]->isFuncDefine == __FALSE) {
                fprintf(stdout,
                        "########## Error at Line#%d: Funcction: '%s' is "
                        "declared but not defined ##########\n",
                        symtab->entry[i]->declarationLine,
                        symtab->entry[i]->name);
                semError = __TRUE;
            }
        }
    ...
    if (attr != 0) {
        ...
        if (category == CONSTANT_t) {
            ...
            if (attr->formalParam->params == 0) {
                ...
                for (current = (attr->formalParam->params->next),
                    previous = (attr->formalParam->params);
                     current != 0;
...
void deleteIdList(struct idNode_sem *idlist) {
    if (idlist == 0)
        ...
        if (idlist->value != 0) free(idlist->value);
        free(idlist);
    } else {
        struct idNode_sem *current, *previous;
        for (current = (idlist->next), previous = idlist; current != 0;
             previous = current, current = (current->next)) {
            if (previous->value != 0) free(previous->value);
        }
        if (previous->value != 0) free(previous->value);
